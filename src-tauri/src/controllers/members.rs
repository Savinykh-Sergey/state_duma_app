...
use crate::database::repositories::members::{
    add_member, delete_member, get_member, get_members, get_retired_members,
    report_experienced_members, update_member,
};
...
pub async fn fetch_member_ui(query: String) -> Result<Members, String> {
    match get_member(query).await {
        Ok(member) => Ok(member),
        Err(reason) => Err(format!(
            "Возникла ошибка при получении члена гос. думы: {}",
            reason
        )),
    }
}
...
#[tauri::command(async)]
pub async fn execute_create_member_ui(
    full_name: String,
    birthday: String,
    experience: i32,
    phone: String,
...
#[tauri::command(async)]
pub async fn execute_edit_member_ui(
    id: String,
    full_name: String,
    birthday: String,
    experience: i32,
    ...
    if let Err(reason) = update_member(
        id_uuid,
        full_name,
        birthday_date,
        experience,
