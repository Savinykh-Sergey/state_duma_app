...
use crate::database::repositories::commissions::{add_commission, delete_commission, get_commission, get_commission_members_by_direction, get_commissions, update_commission};
...
pub async fn fetch_commission_ui(id: String) -> Result<Commissions, String> {
    ...
    match get_commission(id).await {
        ...
        Err(reason) => Err(format!("Возникла ошибка при получении комиссии: {}", reason))
    }
}

#[tauri::command(async)]
pub async fn execute_create_commission_ui(direction: String, chairman_id: String, organizer_id: String) -> Result<(), String> {
    let chairman_id = Uuid::parse_str(&chairman_id).map_err(|e| format!("Неверный формат ID председателя: {}", e))?;
    let organizer_id = Uuid::parse_str(&organizer_id).map_err(|e| format!("Неверный формат ID организатора: {}", e))?;
...
#[tauri::command(async)]
pub async fn execute_edit_commission_ui(id: String, direction: String, chairman_id: String, organizer_id: String) -> Result<(), String> {
    let id = Uuid::parse_str(&id).map_err(|e| format!("Неверный формат ID: {}", e))?;
    let chairman_id = Uuid::parse_str(&chairman_id).map_err(|e| format!("Неверный формат ID председателя: {}", e))?;
    let organizer_id = Uuid::parse_str(&organizer_id).map_err(|e| format!("Неверный формат ID организатора: {}", e))?;
...
#[tauri::command(async)]
pub async fn fetch_members_by_direction_report_ui(direction: String) -> Result<Vec<ReportCommission>, String> {
    println!("{:?}", direction);
    ...
    match get_commission_members_by_direction(direction_enum).await {
        ...
        Err(reason) => Err(format!("Ошибка при получении членов думы по направлению: {}", reason))
    }
}
