...
use crate::database::repositories::commission_members::{
    add_commission_member, edit_commission_member, get_commission_member,
};

#[tauri::command(async)]
pub async fn execute_create_commission_members_ui(
    member_id: String,
    commission_id: String,
) -> Result<(), String> {
    let member_id =
        Uuid::parse_str(&member_id).map_err(|e| format!("Неверный формат ID: {}", e))?;
    let commission_id =
        Uuid::parse_str(&commission_id).map_err(|e| format!("Неверный формат ID: {}", e))?;
...
#[tauri::command(async)]
pub async fn execute_edit_commission_members_ui(
    member_id: String,
    commission_id: String,
) -> Result<(), String> {
    let member_id =
