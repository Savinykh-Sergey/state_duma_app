...
use crate::database::repositories::meetings::{add_meeting, delete_meeting, get_meeting, get_meetings, report_meetings_by_year, update_meeting};
...
#[tauri::command(async)]
pub async fn execute_create_meeting_ui(date: String, duration: i32, commission_id: String) -> Result<(), String> {
    let commission_id = Uuid::parse_str(&commission_id).map_err(|e| format!("Неверный формат ID комиссии: {}", e))?;
...
#[tauri::command(async)]
pub async fn execute_edit_meeting_ui(id: String, date: String, duration: i32, commission_id: String) -> Result<(), String> {
...
pub async fn fetch_meetings_by_year_ui(year: i32) -> Result<Vec<ReportMeetings>, String> {
    match report_meetings_by_year(year).await {
        Ok(meeting) => Ok(meeting),
        Err(reason) => Err(format!("Ошибка при получении заседания по годам: {}", reason))
    }
}