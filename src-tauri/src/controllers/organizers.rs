...
use crate::database::repositories::organizers::{add_organizer, delete_organizer, get_organizer, get_organizers, update_organizer};

#[tauri::command(async)]
pub async fn fetch_organizers_ui() -> Result<Vec<Organizers>, String>{
    match get_organizers().await {
        Ok(organizers) => Ok(organizers),
        Err(reason) => Err(format!("Возникла ошибка при получении организаторов: {}", reason))
    }
}
...
#[tauri::command(async)]
pub async fn execute_create_organizer_ui(full_name: String, birthday: String, phone: String) -> Result<(), String> {
...
#[tauri::command(async)]
pub async fn execute_edit_organizer_ui(id: String, full_name: String, birthday: String, phone: String) -> Result<(), String> {
